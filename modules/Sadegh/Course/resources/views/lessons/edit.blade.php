...
@section('breadcrumb')
    ...
    <li><a href="#" title="بروزرسانی">بروزرسانی درس</a></li>
@endsection

@section('content')
    <div class="row no-gutters">
        <div class="col-12 bg-white">
<p class="box__title">بروزرسانی درس جدید</p>
            <form action="{{ route('lessons.update',[$course->id,$lesson->id]) }}" class="padding-30" method="post" enctype="multipart/form-data">
                ...
                <x-input name="slug" placeholder="نام انگلیسی درس اختیاری" type="text" class="text-left" value="{{ $lesson->slug }}"  />
                <x-input name="time" placeholder="مدت زمان جلسه *" type="number" class="text-left" required value="{{ $lesson->time }}"  />
                <x-input name="number" placeholder="شماره جلسه" type="number" class="text-left" value="{{ $lesson->number }}" />
                ...
                <x-select name="season_id">
                    ...
                    <option value="{{ $season->id }}" @if($season->id == $lesson->season_id) selected @endif>
                        {{ $season->title }}
                    </option>
                    @endforeach
                </x-select>
